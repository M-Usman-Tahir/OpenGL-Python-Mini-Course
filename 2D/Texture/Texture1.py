...
class Renderer:
    ...
    def rotate(self):
        ...
        if self.scaling >= 0.1:
            self.scaling -= 0.001
        rotationMatrix = Matrix44.from_eulers([self.rotateAngle, self.rotateAngle, 0.0], dtype='float32')
        scale = Matrix44.from_scale([self.scaling, self.scaling, 1.0], dtype=np.float32)
        glUniformMatrix4fv(self.rotation_location, 1, GL_FALSE, rotationMatrix*scale)
